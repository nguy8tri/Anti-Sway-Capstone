...
def simulate(X0 = 0.0, x_lim = (0.5588 ** 2 + 0.6096 ** 2) ** 0.5,
    ...
    t, v_ref, x_ref = gen_norm_vel_disturbance(v_lim, x_lim, samples,
                                               vel_option=vel_option)  # Change last parameter for step or ramp
...
if __name__ == "__main__":
    ...
    axs[0].plot(t, x_ref, label="Disturbance")
    axs[0].legend(loc="upper right")

    axs[1].set_ylabel("Distance (m)")
    axs[1].plot(t, x_real, label="Trolley Position")
    axs[1].legend(loc="upper right")

    axs[2].set_ylabel("Velocity (m/s)")
    axs[2].plot(t, v_ref, label="Disturbance")
    axs[2].legend(loc="upper right")

    axs[3].set_ylabel("Velocity (m/s)")
    axs[3].plot(t[:-1], vel_real[:-1], label="Trolley Position")
    axs[3].legend(loc="upper right")

    axs[4].set_ylabel("Angle (deg)")
    axs[4].plot(t, np.rad2deg(theta_real), label="Rope Angle")
    ...
    axs[0].plot(t[:-1], F_real, label="Motor Force")
    axs[0].legend(loc="upper right")

    axs[1].set_ylabel("Torque (N-m)")
    axs[1].plot(t[:-1], T_real, label="Motor Torque")
    axs[1].legend(loc="upper right")

    axs[2].set_ylabel("Angular Velocity\n(rpm)")
    axs[2].plot(t[:-1], rpm_real, label="Motor Velocity")
    axs[2].legend(loc="upper right")

    axs[3].set_ylabel("Current (A)")
    axs[3].plot(t[:-1], A_real, label="Motor Current")
    axs[3].legend(loc="upper right")

    axs[4].set_ylabel("Voltage (V)")
    axs[4].set_xlabel("Time (s)")
    axs[4].plot(t[:-1], V_real, label="Motor Voltage")
    axs[4].legend(loc="upper right")
